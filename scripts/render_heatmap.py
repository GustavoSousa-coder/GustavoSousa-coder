...
heatmap de contribuições no estilo GitHub: grade de caixas arredondadas e
coloridas no calendário clássico de 53 semanas x 7 dias, com uma revelação
"diagonal" (linha após linha, animação CSS que toca uma vez e congela --
sem "brilho" em loop), legenda Less -> More e um rodapé com estatísticas reais.
"""
import datetime
import json
import os
...
# Rampa de verdes estilo GitHub: vazio -> mais brilhante.
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">{USERNAME.lower()}@github: ~/contributions --graph</text>')
    ...
    ly += 24
    parts.append(f'<text x="{PAD}" y="{ly}" font-size="13" fill="{MUTED}">streak atual '
