#!/usr/bin/env python3
"""Gera um SVG animado de contribuições do GitHub (quadrados que acendem um a
um). Funciona sozinho; feito para rodar diariamente via GitHub Action.
Uso: python generate_streak_svg.py [usuario] [output.svg]
"""
import sys, json, os, datetime, urllib.request
USER = sys.argv[1] if len(sys.argv) > 1 else "GustavoSousa-coder"
...
def get_data(user):
    ...
    try:
        ...
        # se a API estiver fora do ar, usa um snapshot local
        here = os.path.join(os.path.dirname(os.path.abspath(__file__)), "contrib.json")
        if os.path.exists(here):
            print("API falhou (%s); usando contrib.json local" % e)
